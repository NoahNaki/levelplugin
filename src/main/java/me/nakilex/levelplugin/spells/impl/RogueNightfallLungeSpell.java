...
public class RogueNightfallLungeSpell implements SpellHandler {
    ...
    public void cast(SpellContext context) {
        ...
        new BukkitRunnable() {
            ...
            @Override
            public void run() {
                if (!caster.isOnline() || !target.isValid() || target.isDead() || tick >= 4) {
                    cancel();
                    return;
                }

                Location center = target.getLocation().clone().add(0.0, 1.0, 0.0);
                double angle = (Math.PI * 2.0 * tick) / 4.0;
                double radius = 1.3;
                ...
                ArcSlashCombatUtil.strike(caster, orbit, orientation, 6.0 + (tick * 0.55), 2.0);

                SpellEffectUtil.applyDirectSpellDamage(plugin, caster, target, 7.2 + (tick * 0.6), true);
                ...
                if (tick == 3) {
                    SpellEffectUtil.applyDirectSpellDamage(plugin, caster, target, 11.8, true);
