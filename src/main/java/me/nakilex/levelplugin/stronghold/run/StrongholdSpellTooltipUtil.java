    ...
    public static void appendSpellEffectLore(List<String> lore, String spellId) {
        ...
        switch (normalized) {
            case "warrior_earthquake", "warrior_earthquake_tremor", "warrior_earthquake_cataclysm" ->
                    appendHighlightedBullet(lore, "Sends a radial shockwave that ripples the ground for ", ChatColor.RED, "AoE melee damage", ".");
            ...
            case "mage_fireball_basic", "mage_fireball_barrage", "mage_fireball_inferno", "mage_fireball_chainlightning" ->
                    appendHighlightedBullet(lore, "Launches projectiles with ", ChatColor.RED, "ranged burst damage", " and quick clear potential.");
            case "blackhole", "blackhole_gravitywell", "blackhole_singularity" ->
                    appendHighlightedBullet(lore, "Pulls enemies into a control zone that deals ", ChatColor.DARK_PURPLE, "stacking tick damage", ".");
            case "meteor", "meteor_double", "meteor_big" ->
                    appendHighlightedBullet(lore, "Calls down a meteor strike for ", ChatColor.RED, "high impact area damage", ".");
            ...
            case "archer_quickshot_basic", "archer_quickshot_seeker", "archer_quickshot_payload" ->
                    appendHighlightedBullet(lore, "Fires precision volleys with ", ChatColor.RED, "rapid ranged hits", ".");
            ...
            case "rogue_sky_ripper", "rogue_sky_ripper_tempest", "rogue_sky_ripper_execution" ->
                    appendHighlightedBullet(lore, "Performs chained strikes for ", ChatColor.RED, "combo burst damage", ".");
    ...
    public static void appendUpgradeDeltaLore(List<String> lore, String baseSpellId, int currentRank, boolean unlock) {
        ...
        switch (normalized) {
            case "warrior_earthquake" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.RED, "damage and ripple radius", ".");
            case "warrior_rupture_cyclone" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.RED, "tick damage and sequence uptime", ".");
            case "warrior_execution_arc" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.RED, "burst damage", ".");
            case "warrior_guarded_resolve" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.AQUA, "blocked hits and guard strength", ".");
            case "mage_heal" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.GREEN, "healing throughput", ".");
            case "blackhole" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.DARK_PURPLE, "pull strength and tick damage", ".");
            case "meteor" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.RED, "impact damage and area pressure", ".");
            case "archer_windguard" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.AQUA, "buff potency and team safety", ".");
            case "archer_arrow_rain", "archer_homing_barrage", "archer_quickshot_basic", "rogue_arc_basic",
                 "rogue_sky_ripper", "rogue_phantom_cross" ->
                    appendHighlightedBullet(lore, "Rank " + currentRank + " → " + nextRank + ": improves ", ChatColor.RED, "damage and consistency", ".");
