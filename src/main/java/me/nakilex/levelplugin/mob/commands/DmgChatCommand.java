...
public class DmgChatCommand implements CommandExecutor {
    ...
    public DmgChatCommand(SettingsManager settingsManager) {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        boolean nowEnabled = !chatToggle.isEnabled(player);
        chatToggle.setEnabled(player, nowEnabled);

        PlayerSettings ps = settingsManager.getSettings(player);
        if (ps.isDmgChatEnabled() != nowEnabled) {
            ps.toggleDmgChat();
        }
