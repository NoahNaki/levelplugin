...
public class PlayerScoreboardManager implements org.bukkit.event.Listener {
    ...
    public void updateBoard(Player player) {
        ...
        if (strongholdActive) {
            StrongholdSurvivalManager.StageStatus status = strongholdSurvivalManager.getStage(id);
            current[idx] = ChatColor.GOLD + "Stronghold Survival";
            if (!current[idx].equals(prev[idx])) {
                setLine(board, obj, idx, line, current[idx]);
            }
            idx++;
            ...
            if (status != null) {
                current[idx] = ChatColor.GRAY + "Wave: " + ChatColor.WHITE + status.wave();
                if (!current[idx].equals(prev[idx])) {
                    setLine(board, obj, idx, line, current[idx]);
                }
                idx++;
