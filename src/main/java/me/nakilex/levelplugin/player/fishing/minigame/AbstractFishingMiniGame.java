...
public abstract class AbstractFishingMiniGame implements FishingMiniGame {
    ...
    protected final void actionBar(String message) {
        actionBar(LegacyComponentSerializer.legacySection().deserialize(message));
    }

    protected final void actionBar(Component component) {
        player.sendActionBar(component);
    }

    protected final boolean useResourcePack() {
        return FishingResourcePackManager.canRenderGlyphUi(plugin);
    }

    protected final boolean useFallbackTextUi() {
        return plugin.getConfig().getBoolean("fishing-mini-games.resource-pack.fallback-text-ui", true);
    }
