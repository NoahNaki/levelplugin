...
public class TownStageManager {
    ...
    private final Map<String, TownStage> stages = new HashMap<>();
    ...
    public TownStage getStage(String name) {
    ...
    public void createStage(String name, int level, int stage, Location p1, Location p2) {
        ...
        stages.put(name.toLowerCase(), new TownStage(name.toLowerCase(), level, stage, p1, p2, npcs));
        saveConfig();
    }

    public boolean removeStage(String name) {
        if (stages.remove(name.toLowerCase()) != null) {
            saveConfig();
            return true;
        }
        return false;
    ...
    public void spawnForStage(int level, int stage) {
        for (TownStage ts : stages.values()) {
            if (ts.level == level && ts.stage == stage) {
                var list = spawnedNPCs.computeIfAbsent(ts.name, k -> new java.util.ArrayList<>());
    ...
    private void loadFromConfig() {
        ...
        for (String key : config.getConfigurationSection("stages").getKeys(false)) {
            ...
            stages.put(key.toLowerCase(), new TownStage(key.toLowerCase(), level, stage, p1, p2, npcs));
        }
    }
    ...
    private void saveConfig() {
        ...
        for (TownStage st : stages.values()) {
            String base = "stages." + st.name + ".";
            Location p1 = st.pos1;
            Location p2 = st.pos2;
            config.set(base + "world", p1.getWorld().getName());
            config.set(base + "level", st.level);
            config.set(base + "stage", st.stage);
