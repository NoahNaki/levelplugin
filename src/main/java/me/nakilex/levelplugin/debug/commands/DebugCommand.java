 ...
 *   <li><code>nexoscan</code> – scans the "mmorpg" world for Nexo furniture</li>
 * </ul>
 */
public class DebugCommand implements TabExecutor {
    ...
    private final NexoScanCommand nexoScanCommand;

    public DebugCommand(PlayerToggleManager mobDebugManager,
                        PlayerScoreboardManager scoreboardManager,
                        NexoScanCommand nexoScanCommand) {
        this.mobDebugManager = mobDebugManager;
        this.scoreboardManager = scoreboardManager;
        this.nexoScanCommand = nexoScanCommand;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("Usage: /debug <mobinfo|tps|nexoscan>");
            return true;
        }
        ...
        switch (sub) {
            ...
            case "nexoscan":
                // Delegate to the existing NexoScanCommand implementation
                return nexoScanCommand.onCommand(sender, cmd, label, new String[0]);
